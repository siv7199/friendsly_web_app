...
export async function getCreatorAnalyticsSnapshot(creatorId: string, range: AnalyticsRangeKey): Promise<CreatorAnalyticsSnapshot> {
  ...
  const [viewsRes, bookingsRes, liveRes] = await Promise.all([
    ...
    supabase
      .from("bookings")
      .select("id, created_at, scheduled_at, duration, status, price")
      .eq("creator_id", creatorId)
      .gte("created_at", startDate.toISOString()),
    supabase
      .from("live_sessions")
      .select("id, live_queue_entries(id, joined_at, ended_at, status, amount_charged)")
      .eq("creator_id", creatorId),
  ]);
  ...
  const conversionRate = conversionBase > 0
    ? Math.min(100, Math.round((((bookingsCreated + liveQueueJoins) / conversionBase) * 100) * 10) / 10)
    : 0;
